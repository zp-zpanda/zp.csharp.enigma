...
namespace ZP.CSharp.Enigma.Helpers
{
    ...
    */
    public static class ReflectorMappingHelpers
    {
        ...
        */
        public static TSingle[] ToDomainBased<TSingle>(TSingle[] oneTwoMapping, TSingle[] domain)
        {
            var maps = new TSingle[oneTwoMapping.Length];
            oneTwoMapping
                ...
                .Select(data => new[]{(Item: data.First(), Index: Array.IndexOf(domain, data.Last())), (Item: data.Last(), Index: Array.IndexOf(domain, data.First()))})
                .SelectMany(m => m)
                .ToList()
        ...
        */
        public static TSingle[] ToOneTwo<TSingle>(TSingle[] domainBasedMapping, TSingle[] domain)
            where TSingle : IEqualityOperators<TSingle, TSingle, bool>
        {
            var mapsWithIndexes = domainBasedMapping
                .Select((item, index) => (SelfIndex: index, DomainIndex: Array.IndexOf(domain, item)))
            ...
            return maps.Join(
                ...
                (map1, map2) => new[]{map1, map2}.OrderBy(m => Array.IndexOf(domain, m.First.Item)).First())
